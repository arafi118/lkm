    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="0" style="font-size: 11px;">
        ...
        @foreach ($arus_kas as $ak)
            ...
            @foreach ($ak->child as $child)
                ...
                @foreach ($lokasi_list as $lokasiId)
                    @php
                        $nilaiLokasi = $data_perlokasi[$lokasiId]['child_values'][$child->rekening] ?? 0;
                        $namaKec     = $kecamatan_list[$lokasiId]->nama_kec ?? $lokasiId;
                    @endphp
                    <tr>
                        <td>&nbsp;</td>
                        <td>&nbsp;&nbsp;&nbsp;&nbsp;{{ $child->nama_akun }} {{ $namaKec }}</td>
                        <td align="right">{{ number_format($nilaiLokasi, 2) }}</td>
                    </tr>
                @endforeach
            @endforeach
